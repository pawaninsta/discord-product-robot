...
/**
 * Condense a long description to fit within a character limit.
 * @param {string} description - The original description
 * @param {number} maxLength - Maximum character length (default 280)
 * @returns {Promise<string>} - Condensed description
 */
export async function condenseDescription(description, maxLength = 280) {
  ...
  const resp = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: `You are a concise copywriter. Condense the given product description to under ${maxLength} characters while preserving the key selling points. Keep the same tone and style. Return ONLY the condensed text, nothing else.`
      },
      {
        role: "user",
        content: description
  ...
  const condensed = resp?.choices?.[0]?.message?.content?.trim() || description;
  
  // If still too long, truncate at last complete sentence
  if (condensed.length > maxLength) {
    const truncated = condensed.substring(0, maxLength);
...
## DESCRIPTION FORMAT - TELL THE UNIQUE STORY (3-4 sentences, MAX 280 characters)
Your description MUST be CONCISE. Keep it under 280 characters. Answer: "What makes THIS bottle special?"
